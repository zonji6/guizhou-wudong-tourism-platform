...
- [ ] 商品订单校验数量和取货点，初始为待取货；餐食订单校验到店时间与人数，初始为待到店；住宿预约校验 `checkInDate`、`checkOutDate`、`roomCount`、`peopleCount`，初始仍为待确认。
...
- [ ] 住宿提案补齐入住/离店日期、房间数和人数，并与 Java API、Web/小程序严格解析器同步升级协议版本；不能只在现有 `cardVersion=2.0` 增加字段。提案存储方式不在本轮补充中定案。
...
- [ ] 按后端规格进行手工 API 验收：三类订单状态、住宿四字段、访客隔离、社区路线攻略、AI 待确认餐食/住宿、示意地图边界和后台脱敏摘要。
