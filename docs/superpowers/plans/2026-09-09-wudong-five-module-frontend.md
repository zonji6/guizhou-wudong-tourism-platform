...
**跨端需求修订：** 用户已确认 Web 与小程序的个人行程、AI 草稿和订单需要互通。下文原本地 `visitorId` 方案保留为旧基线，不能按两端独立身份直接完成验收；须先在技术讨论中确定身份关联与请求凭据，再同步调整两端 API、我的页面和相关任务。不据此增加完整账号体系或商家登录，详见整合开发文档第 4.1、5.3 节。
