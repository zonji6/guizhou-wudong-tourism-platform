...
- [ ] 详情页按类型只显示一个主动作：商品数量/取货点后下单，餐食到店时间/人数后下单，住宿填写入住日期 `checkInDate`、离店日期 `checkOutDate`、房间数 `roomCount` 和人数 `peopleCount` 后预约。
...
- [ ] 逐项按前端规格执行手工验收：模块入口、三类订单、住宿四字段、社区路线攻略、水彩示意地图及其标识、我的订单与后台状态。
...
- 下一阶段同步更新 Java API、住宿 AI 提案和两端严格解析器，并升级协议版本；不能只给现有 `cardVersion=2.0` 加键。AI 提案保存方案仍保留原讨论项。
