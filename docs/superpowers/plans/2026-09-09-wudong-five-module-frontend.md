...
**跨端需求修订：** 用户已确认 Web 与小程序的个人行程、AI 草稿和订单需要互通，并选择轻量账号密码，两端登录同一平台账号。下文本地 `visitorId`、匿名访客任务及游客侧“不做登录”表述保留为旧基线，不能直接作为最终实现或验收要求；须先确定账号创建与会话方案，再修订两端 API、我的页面和相关任务。不据此增加完整注册找回体系或商家登录，详见整合开发文档第 4.1、5.3 节。
