...
后台边界已由用户补充：仅向开发和运营人员开放，游客顶栏右侧为“我的”，移除运营后台入口。此要求覆盖此前未做后台权限的旧边界；普通自助注册不能获得管理权限。后台入口与服务端授权均须区分，角色已确认分为游客与后台管理人员，开发者和运营者各用独立账号、共用一档既定后台业务权限，不增加复杂权限配置；管理账号创建与授权方式仍待讨论。
