...
用户进一步确认开放最简自助注册：用户名与密码必填，昵称可选，暂不接短信或邮箱验证；不是只能使用预置演示账号。该选择不自动增加商家账号、完整找回体系或后台角色权限。密码重置方式、登录会话技术、旧匿名数据处理和表结构仍待讨论；没有实施注册、登录或迁移数据。
...
讨论结果：用户选择轻量账号密码，不采纳此前优先讨论微信身份的初步倾向。微信与匿名配对部分仅作备选调研保留，不进入本期实现；不为未采用的方案新增配对票据或微信身份表。登录会话方式、表结构和接口尚未锁定。
