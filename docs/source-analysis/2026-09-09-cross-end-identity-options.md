...
后台边界已由用户补充：仅向开发和运营人员开放，游客顶栏右侧为“我的”，移除运营后台入口。此要求覆盖此前未做后台权限的旧边界；普通自助注册不能获得管理权限。后台入口与服务端授权均须区分，角色已确认分为游客与后台管理人员，开发者和运营者各用独立账号、共用一档既定后台业务权限，不增加复杂权限配置；后台账号已确认由开发通过初始化脚本创建并赋予既定后台业务权限，不开放后台自助注册或人员管理、授权页面，账号凭据不写入公开仓库；不改变游客自助注册，密码恢复细节仍待讨论。

游客端与后台登录隔离已确认：同一浏览器内也须分别登录，游客登录不自动进入后台，后台须校验独立后台登录与管理权限；两边分别续期、撤销和退出，互不影响，分别执行从各自登录成功起最长 7 天、8 小时的绝对期限。此决定不改变 Web 与小程序登录同一游客账号后访问同一份个人数据的要求，也不拆两套用户库、两个项目或新增小程序后台。登录用途与凭据保存、传输的实现方式仍待细化。

登录时机已确认：浏览首页、商品、菜单、民宿及公开社区内容无需登录；保存行程或草稿、提交订单、发布社区内容、查看个人记录时要求登录。登录成功后回到原来的操作位置，保留本次填写内容，但不自动提交订单；订单仍须用户确认。保留本次输入不等于迁移或合并历史匿名数据。

2026-09-10 用户确认 AI 向导采用“先体验，保存时再登录”：未登录游客可以询问乌东文化、生成并预览行程，使用公开资料而不读取任何账号的私人数据。保存行程或草稿、提交订单和查看个人记录仍须登录，匿名预览不直接成为“我的”中的已保存成果，也不能以匿名会话标识访问个人接口。调用限额、匿名会话的保存期限与访问凭据、登录后本次预览的衔接方式另行设计；不因开放预览就默认批准历史聊天迁移；跨端继续规划按下述已确认范围执行。

2026-09-10 用户确认跨端继续规划的范围：同一账号在 Web 保存行程后，小程序可打开，并由 AI 基于最新已保存行程继续修改；反向接续遵循同一规则。首版暂不跨端同步逐条聊天记录，换端后不能据此查看另一端完整问答；当前会话的上下文记忆保留。这不改变行程、草稿和订单互通，不要求两端共享同一 `threadId`，也不把未保存的候选建议当作跨端成果。AI 修改仍先预览再由用户采用；行程并发修改已确认采用版本号与条件更新，旧版本拒绝覆盖，保留本页未保存编辑并提示读取最新版后重新调整，首版不自动合并。跨端更新通知方式继续细化。
