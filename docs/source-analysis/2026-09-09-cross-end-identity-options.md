...
2026-09-10 用户确认 AI 向导采用“先体验，保存时再登录”：未登录游客可以询问乌东文化、生成并预览行程，使用公开资料而不读取任何账号的私人数据。保存行程或草稿、提交订单和查看个人记录仍须登录，匿名预览不直接成为“我的”中的已保存成果，也不能以匿名会话标识访问个人接口。调用限额、匿名会话的保存期限与访问凭据、登录后本次预览的衔接方式另行设计；不因开放预览就默认批准历史聊天迁移或跨端聊天同步。
