...
2026-09-10 用户确认跨端继续规划的范围：同一账号在 Web 保存行程后，小程序可打开，并由 AI 基于最新已保存行程继续修改；反向接续遵循同一规则。首版暂不跨端同步逐条聊天记录，换端后不能据此查看另一端完整问答；当前会话的上下文记忆保留。这不改变行程、草稿和订单互通，不要求两端共享同一 `threadId`，也不把未保存的候选建议当作跨端成果。AI 修改仍先预览再由用户采用；并发编辑与更新通知方式继续细化。
