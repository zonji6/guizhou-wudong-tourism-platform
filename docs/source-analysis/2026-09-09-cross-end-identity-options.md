...
用户进一步确认开放最简自助注册：用户名与密码必填，昵称可选，暂不接短信或邮箱验证；不是只能使用预置演示账号。不自动增加商家账号或完整找回体系。首版登录技术已选择 Spring Security + JWT，密码重置、会话细节、旧匿名数据处理和表结构仍待讨论；没有实施注册、登录或迁移数据。

后台边界已由用户补充：仅向开发和运营人员开放，游客顶栏右侧为“我的”，移除运营后台入口。此要求覆盖此前未做后台权限的旧边界；普通自助注册不能获得管理权限。后台入口与服务端授权均须区分，角色已确认分为游客与后台管理人员，开发者和运营者各用独立账号、共用一档既定后台业务权限，不增加复杂权限配置；后台账号已确认由开发通过初始化脚本创建并赋予既定后台业务权限，不开放后台自助注册或人员管理、授权页面，账号凭据不写入公开仓库；不改变游客自助注册，登录隔离与密码恢复细节仍待讨论。
