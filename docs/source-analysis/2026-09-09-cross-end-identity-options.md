...
日期：2026-09-09。用户已选择轻量账号密码作为本期跨端身份入口；以下保留方案调研，选择确认不代表接入完成。

## 已确认的选择

Web 与原生微信小程序使用同一套平台用户名、密码登录，个人行程、草稿和订单归属于同一内部用户。两端各持登录状态，不能以共享固定游客 ID 替代身份校验。本期不采用匿名配对或微信登录；微信身份凭据与网站扫码登录条件不再是此次账号方案的前置条件，但小程序本身的开发配置和真机网络仍需单独处理。

用户进一步确认开放最简自助注册：用户名与密码必填，昵称可选，暂不接短信或邮箱验证；不是只能使用预置演示账号。不自动增加商家账号或完整找回体系。首版登录会话已选择 Spring Security + Spring Session Redis，密码重置、会话细节、旧匿名数据处理和表结构仍待讨论；没有实施注册、登录或迁移数据。

后台边界已由用户补充：仅向开发和运营人员开放，游客顶栏右侧为“我的”，移除运营后台入口。此要求覆盖此前未做后台权限的旧边界；普通自助注册不能获得管理权限。后台入口与服务端授权均须区分，具体管理角色分档、账号创建与授权方式待讨论。

登录时机已确认：浏览首页、商品、菜单、民宿及公开社区内容无需登录；保存行程或草稿、提交订单、发布社区内容、查看个人记录时要求登录。登录成功后回到原来的操作位置，保留本次填写内容，但不自动提交订单；订单仍须用户确认。保留本次输入不等于迁移或合并历史匿名数据，AI 匿名试用及会话访问边界也未因此选定。

可选注册昵称不自动替代既有社区作者展示规则，是否用于公开展示后续确认；昵称不是身份凭据，也不据此增加头像、个人资料编辑或角色选择。
...
已确认由 Java 的 Spring Security 与 Spring Session Redis 管理登录及会话，MySQL 保存用户归属和业务数据；不增加未选用的配对挑战机制。Spring Session 官方提供 Redis 支持的 REST 会话及请求头会话传递示例，具体版本须匹配现有 Spring Boot 工程，不能照搬最新示例。[Spring Session REST 官方说明](https://docs.spring.io/spring-session/reference/3.4/guides/java-rest.html)
