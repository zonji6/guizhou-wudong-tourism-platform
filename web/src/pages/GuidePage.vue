...
<template>
  <main class="page v3-guide-page">
    ...
    <section v-if="workspaceReady" class="v3-guide-workspace" aria-label="乌东向导工作台">
      ...
      <div class="v3-guide-suggestions"><button type="button" :disabled="busy" @click="askSuggestion('周末两位，安排两天一夜的苗族文化茶旅')">两天一夜茶旅</button><button type="button" :disabled="busy" @click="askSuggestion('推荐适合亲子的苗绣体验')">亲子苗绣</button><button type="button" :disabled="busy" @click="askSuggestion('介绍一下乌东的茶旅体验')">认识乌东茶旅</button></div>
