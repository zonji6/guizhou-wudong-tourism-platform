...
function actionLabel(action) {
  if (!userMode.value && action?.action === 'SAVE_ITINERARY') return '登录后保存行程'
  if (!userMode.value && ['ADOPT_FOOD_DRAFT', 'ADOPT_STAY_DRAFT'].includes(action?.action)) return '登录后保存草稿'
...
function openRecommendation(item) {
  followAction({ action: 'OPEN_DETAIL', targetType: item?.targetType, targetId: item?.targetId })
}
...
async function followAction(action) {
  ...
  const focusKey = { PRODUCT: 'productId', FOOD: 'foodId', STAY: 'roomId', PLACE: 'placeId' }[action?.targetType]
  if (focusKey && action?.targetId) destination += `?${focusKey}=${encodeURIComponent(action.targetId)}`
