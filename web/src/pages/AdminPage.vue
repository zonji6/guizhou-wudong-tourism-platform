...
const publishedCount = key => (data.operations[key] || []).filter(item => item.catalogStatus === 'PUBLISHED').length
const allOrders = computed(() => ['products', 'foods', 'stays'].flatMap(kind => (data.operations[`orders:${kind}`] || []).map(order => ({ ...order, kind }))))
const foodOrderMerchants = computed(() => [...new Set(allOrders.value.filter(order => order.kind === 'foods').map(order => order.merchantName).filter(Boolean))].sort((left, right) => left.localeCompare(right, 'zh-CN')))
const filteredOrders = computed(() => allOrders.value.filter(order => (orderKindFilter.value === 'ALL' || order.kind === orderKindFilter.value) && (!foodMerchantFilter.value || order.kind !== 'foods' || order.merchantName === foodMerchantFilter.value)))
const dashboardCards = computed(() => [
  { label: '商品目录', value: count('catalog:products'), note: `已发布 ${publishedCount('catalog:products')} 条` },
  { label: '餐食菜品', value: count('catalog:foods'), note: `已发布 ${publishedCount('catalog:foods')} 条 · ${count('catalog:merchants')} 家店铺` },
  { label: '住宿房型', value: count('catalog:room-types'), note: `已发布 ${publishedCount('catalog:room-types')} 条 · ${count('catalog:stays')} 处住宿` },
...
const contentComposition = computed(() => {
  const entries = [
    { label: '商品', value: count('catalog:products'), color: '#c98c63' },
    { label: '餐食', value: count('catalog:foods'), color: '#397f7a' },
    { label: '住宿房型', value: count('catalog:room-types'), color: '#6b9db1' },
    { label: '地点', value: count('catalog:places'), color: '#8cab79' }
  ]
  const total = entries.reduce((sum, item) => sum + item.value, 0)
  return entries.map(item => ({ ...item, percent: total ? Math.max(4, Math.round(item.value / total * 100)) : 0 }))
})
const catalogRecordTotal = computed(() => ['products', 'foods', 'room-types', 'places'].reduce((total, kind) => total + count(`catalog:${kind}`), 0))
const publishedCatalogTotal = computed(() => ['products', 'foods', 'room-types', 'places'].reduce((total, kind) => total + publishedCount(`catalog:${kind}`), 0))
  ...
  <div class="v3-admin-shell">
    ...
    <main class="v3-admin-main">
      ...
      <template v-else>
        ...
        <template v-else>
          <section v-if="tab === 'dashboard'" class="v3-admin-dashboard"><div class="v3-admin-hero"><div><p class="eyebrow">今日看板 · 数据随刷新更新</p><h2>沿着溪流，看见乌东的每一次相遇</h2><p>这是本机答辩演示的运营视图：目录、订单、社区与知识分别来自业务接口，不展示虚构 GMV 或实时客流。</p></div><span>乌东<br>运营</span></div><div class="v3-admin-metrics"><article v-for="card in dashboardCards" :key="card.label"><small>{{ card.label }}</small><strong>{{ card.value }}</strong><p>{{ card.note }}</p></article></div><section class="v3-admin-composition"><header><div><p class="eyebrow">目录内容构成</p><h2>用资源结构，而不是虚构指标说明平台厚度</h2></div><small>目录 {{ catalogRecordTotal }} 条 · 已发布 {{ publishedCatalogTotal }} 条</small></header><div v-for="item in contentComposition" :key="item.label" class="v3-composition-row"><span>{{ item.label }}</span><i><b :style="{ width: `${item.percent}%`, background: item.color }"></b></i><strong>{{ item.value }}</strong></div></section><section class="v3-admin-pulse"><header><div><p class="eyebrow">处理提醒</p><h2>订单与知识状态</h2></div><button class="ghost" @click="tab = 'orders'">查看订单</button></header><div><article><b>{{ allOrders.filter(item => /^PENDING/.test(item.status || '')).length }}</b><span>待推进订单</span><small>仅展示接口实际返回的订单</small></article><article><b>{{ data.documents.filter(item => item.currentTaskId).length }}</b><span>知识发布进行中</span><small>发布任务不等于已经生效</small></article><article><b>{{ publishedCount('catalog:places') }} / {{ count('catalog:places') }}</b><span>已发布地图示意节点</span><small>不代表真实导航与比例</small></article></div></section></section>
          <section v-else-if="tab === 'orders'" class="v3-admin-orders"><header class="v3-section-heading"><div><p class="eyebrow">三类订单</p><h2>按状态推进服务</h2><p>商品、同店餐食、住宿各自独立处理；状态变更由服务端校验当前状态。</p></div></header><div class="v3-order-filters"><button v-for="item in [{ key: 'ALL', label: '全部' }, { key: 'products', label: '商品' }, { key: 'foods', label: '餐食' }, { key: 'stays', label: '住宿' }]" :key="item.key" type="button" :class="{ active: orderKindFilter === item.key }" @click="orderKindFilter = item.key; if (item.key !== 'foods') foodMerchantFilter = ''">{{ item.label }}</button><label v-if="orderKindFilter === 'ALL' || orderKindFilter === 'foods'">餐饮店铺<select v-model="foodMerchantFilter"><option value="">全部店铺</option><option v-for="merchant in foodOrderMerchants" :key="merchant" :value="merchant">{{ merchant }}</option></select></label><small>当前 {{ filteredOrders.length }} / {{ allOrders.length }} 笔订单</small></div><article v-for="order in filteredOrders" :key="`${order.kind}-${order.id}`" class="v3-admin-order"><header><span>{{ order.kind === 'products' ? '商品取货' : order.kind === 'foods' ? '到店餐食' : '住宿入住' }}</span><b>{{ order.status }}</b></header><h3>{{ displayName(order) }}</h3><p>{{ order.createdAt || '创建时间待返回' }} · {{ order.demoData ? '演示订单' : '业务订单' }}</p><ul v-if="order.kind === 'foods'" class="v3-admin-food-lines"><li v-for="line in order.items || []" :key="line.sequence">{{ line.foodItemName }} × {{ line.quantity }}<small>¥{{ line.lineAmount }}</small></li></ul><p v-if="order.kind === 'foods'">{{ order.merchantName }} · {{ order.visitAt || '到店时间待确认' }} · {{ order.peopleCount || '人数待确认' }} 人</p><footer><small>#{{ order.id }}</small><button v-if="orderAction(order)" class="primary" :disabled="statusUpdating" @click="advanceOrder(order)">{{ statusUpdating === `${order.kind}:${order.id}` ? '正在更新…' : orderAction(order)[1] }}</button><span v-else>当前状态无需后台推进</span></footer></article><p v-if="!filteredOrders.length" class="v3-state">当前筛选条件下暂无订单。</p></section>
