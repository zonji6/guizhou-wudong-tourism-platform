  ...
  <div class="v3-admin-shell">
    ...
    <main class="v3-admin-main">
      ...
      <template v-else>
        ...
        <template v-else>
          <section v-if="tab === 'dashboard'" class="v3-admin-dashboard"><div class="v3-admin-hero"><div><p class="eyebrow">今日看板 · 数据随刷新更新</p><h2>沿着溪流，看见乌东的每一次相遇</h2><p>这是本机答辩演示的运营视图：目录、订单、社区与知识分别来自业务接口，不展示虚构 GMV 或实时客流。</p></div><span>乌东<br>运营</span></div><div class="v3-admin-metrics"><article v-for="card in dashboardCards" :key="card.label"><small>{{ card.label }}</small><strong>{{ card.value }}</strong><p>{{ card.note }}</p></article></div><section class="v3-admin-composition"><header><div><p class="eyebrow">公开内容构成</p><h2>用资源结构，而不是虚构指标说明平台厚度</h2></div><small>随目录刷新更新</small></header><div v-for="item in contentComposition" :key="item.label" class="v3-composition-row"><span>{{ item.label }}</span><i><b :style="{ width: `${item.percent}%`, background: item.color }"></b></i><strong>{{ item.value }}</strong></div></section><section class="v3-admin-pulse"><header><div><p class="eyebrow">处理提醒</p><h2>订单与知识状态</h2></div><button class="ghost" @click="tab = 'orders'">查看订单</button></header><div><article><b>{{ allOrders.filter(item => /^PENDING/.test(item.status || '')).length }}</b><span>待推进订单</span><small>仅展示接口实际返回的订单</small></article><article><b>{{ data.documents.filter(item => item.currentTaskId).length }}</b><span>知识发布进行中</span><small>发布任务不等于已经生效</small></article><article><b>{{ count('catalog:places') }}</b><span>地图示意节点</span><small>不代表真实导航与比例</small></article></div></section></section>
